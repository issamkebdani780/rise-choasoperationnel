    ...
    <section id="dashboard" className="py-24 lg:py-32 bg-slate-50 dark:bg-slate-900/50 transition-colors duration-300">
      <div className="container mx-auto px-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[24px] sm:rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden">
          {/* Tab bar */}
          <div className="flex items-center gap-1 overflow-x-auto no-scrollbar px-3 sm:px-4 pt-3 sm:pt-4 pb-0 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            {tabs.map(tab => (
              <button
                key={tab.key}
                onClick={() => setActiveTab(tab.key)}
                className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-bold rounded-t-xl whitespace-nowrap transition-all shrink-0 border-b-2 -mb-[1px] ${activeTab === tab.key ? 'border-primary text-primary bg-white dark:bg-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-white/50 dark:hover:bg-slate-800/50'}`}
              >
                <span>{tab.icon}</span>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
                <div className="flex-1 flex items-center gap-2 px-4 py-2.5 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                  ...
                  <input 
                    ...
                    className="flex-1 bg-transparent border-none outline-none text-sm text-body dark:text-slate-300 placeholder:text-slate-400 min-w-0"
                  />
                  {searchQuery && (
                    ...
                    filteredContent.map((row, i) => (
                      <div key={i} className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl space-y-3">
                        ...
                        <div className="flex items-center justify-between text-sm">
                          <div>
                            <span className="text-body dark:text-slate-300">{row.client}</span>
                            <span className="text-slate-300 dark:text-slate-600 mx-1.5">·</span>
                            <span className="text-slate-400">{row.agent}</span>
                          </div>
                          <span className="font-bold text-heading dark:text-white">{row.amount}</span>
                        </div>
                      </div>
                    ))
                  )}
            ...
            {activeTab === 'stock' && (
              <div className="space-y-3">
                {filteredContent.length === 0 ? (
                  ...
                  filteredContent.map((row, i) => (
                    <div key={i} className="flex flex-wrap sm:flex-nowrap items-center gap-3 sm:gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl group">
